...
- ipnsort catches up to intel_avx512 at ~1m input size, despite only using SSE2 instructions and using no hardware specific code, and while caring about binary-size and compile-times, sometimes at the cost of performance.
...
- Both manually vectorized implementations can nearly double their throughput. vqsort only demonstrates this on Linux for this size, and shows a smaller relative gain on Windows.
...
- On Windows below 10k inputs and on Linux bbelow 200, intel_avx512 is faster than vqsort for every pattern.
...
- As noted earlier, intel_avx512 shows worse than `O(N * log(N))` scaling for random_p5, making ipnsort ~15x times faster on Windows and ~19x on Linux for input size 10m.
- Both intel_avx512 and vqsort are unable to leverage already sorted inputs. While the pdqsort derived implementations can handle this case in `O(N)`. At input size 100k, ipnsort is ~20x faster on Windows and ~15x on Linux. And at input size 10m, it is ~10x faster on Windows and ~16x on Linux, where the 10m case will be mostly limited by main memory bandwidth, of which the Server Skylake implementation has more.
...
crumsort mirrors the C standard library `qsort` interface, which asks for a comparison function `int (*comp)(const void *, const void *)`. While a lot of code implement this with `return a - b;`, such code leads to UB for some integers and is not generic. A simple implementation as presented on the cppreference website is this:
...
However depending on your compiler, this will generate branches. In turn, this causes branch miss-prediction which is undesirable, especially for code that wants to exploit ILP. Another alternative to write this in a way that generates good branchless code-gen, especially with Clang is this:
...
Out of the ~500k calls to `slice::sort` 71.6% were `len == 0`, 22.8% were `len == 1` and in total 99+% were `len <= 20`. Together they account for ~50% of the time spent in `slice::sort`.
