...
where
    F: FnMut(&T, &T) -> bool,
{
    // SAFETY: offsets_ptr must be able to hold block.len() writes.
    ...
    if block_len >= UNROLL_SIZE {
        ...
        while unroll_offset < unroll_end {
            // SAFETY: TODO
            unsafe {
                let mut partition_mask = 0u8;
                for i in 0..UNROLL_SIZE {
                    partition_mask |= (!is_less(v_block.get_unchecked(unroll_offset + i), pivot)
                        as u8)
                        .wrapping_shl(i as u32);
                }

                offsets_ptr = update_offsets_ptr(partition_mask, offsets_ptr, unroll_offset);
            }
...
//     let elem_rev_ptr = v_block.as_ptr().add(block_len - 1);
...
//                 let mut partition_mask = 0u8;
//                 for i in 0..UNROLL_SIZE {
//                     partition_mask |= (is_less(&*elem_rev_ptr.sub(unroll_offset + i), pivot) as u8)
//                         .wrapping_shl(i as u32);
//                 }
...
//             // Branchless comparison.
//             *offsets_ptr = i as u8;
//             offsets_ptr = offsets_ptr.wrapping_add(is_less(&*elem_rev_ptr.sub(i), pivot) as usize);
//         }
//     }
