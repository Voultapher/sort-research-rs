...
uint32_t sort_by_impl(T* data, size_t len, F cmp_fn, uint8_t* ctx) noexcept {
    try {
        ...
        SortT<CompWrapper<T, F>*>{}.sort(reinterpret_cast<CompWrapper<T, F>*>(data),
...
uint32_t powersort_stable_ffi_string_by(FFIString* data,
                                        ...
                                        CompResult (*cmp_fn)(const FFIString&,
                                                             ...
                                                             uint8_t*),
                                        uint8_t* ctx) {
    return sort_by_impl<FFIString, powersort>(reinterpret_cast<FFIStringCpp*>(data), len, cmp_fn,
                                              ctx);
}
...
uint32_t powersort_stable_f128_by(F128* data,
                                  ...
                                  uint8_t* ctx) {
    return sort_by_impl<F128, powersort>(reinterpret_cast<F128Cpp*>(data), len, cmp_fn, ctx);
}

// --- 1k ---

void powersort_stable_1k(FFIOneKibiByte* data, size_t len) {
    powersort<FFIOneKiloByteCpp*>{}.sort(reinterpret_cast<FFIOneKiloByteCpp*>(data),
...
uint32_t powersort_stable_1k_by(FFIOneKibiByte* data,
                                ...
                                CompResult (*cmp_fn)(const FFIOneKibiByte&,
                                                     ...
                                                     uint8_t*),
                                uint8_t* ctx) {
    return sort_by_impl<FFIOneKibiByte, powersort>(reinterpret_cast<FFIOneKiloByteCpp*>(data), len,
                                                   cmp_fn, ctx);
}
...
void powersort_4way_stable_ffi_string(FFIString* data, size_t len) {
    powersort_4way<FFIStringCpp*>{}.sort(reinterpret_cast<FFIStringCpp*>(data),
                                         reinterpret_cast<FFIStringCpp*>(data) + len);
}

uint32_t powersort_4way_stable_ffi_string_by(FFIString* data,
                                             size_t len,
                                             CompResult (*cmp_fn)(const FFIString&,
                                                                  const FFIString&,
                                                                  uint8_t*),
                                             uint8_t* ctx) {
    return sort_by_impl<FFIString, powersort_4way>(reinterpret_cast<FFIStringCpp*>(data), len,
                                                   cmp_fn, ctx);
}
...
uint32_t powersort_4way_stable_f128_by(F128* data,
                                       ...
                                       CompResult (*cmp_fn)(const F128&, const F128&, uint8_t*),
                                       uint8_t* ctx) {
    return sort_by_impl<F128, powersort_4way>(reinterpret_cast<F128Cpp*>(data), len, cmp_fn, ctx);
}

// --- 1k ---

void powersort_4way_stable_1k(FFIOneKibiByte* data, size_t len) {
    powersort_4way<FFIOneKiloByteCpp*>{}.sort(reinterpret_cast<FFIOneKiloByteCpp*>(data),
                                              reinterpret_cast<FFIOneKiloByteCpp*>(data) + len);
}
...
uint32_t powersort_4way_stable_1k_by(FFIOneKibiByte* data,
                                     ...
                                     CompResult (*cmp_fn)(const FFIOneKibiByte&,
                                                          ...
                                                          uint8_t*),
                                     uint8_t* ctx) {
    return sort_by_impl<FFIOneKibiByte, powersort_4way>(reinterpret_cast<FFIOneKiloByteCpp*>(data),
                                                        len, cmp_fn, ctx);
}
}  // extern "C"
