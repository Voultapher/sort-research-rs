...
#include "vqsort.h"      // SortDescending
// #include "../print.h"
// #include "shared.h"
...
struct KeyLane {
  ...
  std::string KeyString() const {
    // char string100[100];
    // hwy::detail::TypeName(hwy::detail::MakeTypeInfo<KeyType>(), 1,
    // string100); return string100;
...
struct OrderAscending : public KeyLane<T> {
  ...
  template <class D>
  HWY_INLINE Vec<D> FirstOfLanes(D d,
                                 Vec<D> v,
  ...
  template <class D>
  HWY_INLINE Vec<D> LastOfLanes(D d,
                                Vec<D> v,
                                T* HWY_RESTRICT /* buf */) const {
