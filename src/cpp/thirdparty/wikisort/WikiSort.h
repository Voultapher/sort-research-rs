...
// simulate comparisons that have a bit more overhead than just an inlined (int
// < int) (so we can tell whether reducing the number of comparisons was worth
// the added complexity)
#define SLOW_COMPARISONS false

// if true, test against std::__inplace_stable_sort() rather than
// std::stable_sort()
#define TEST_INPLACE false

// whether to give WikiSort a full-size cache, to see how it performs when given
// more memory
...
#if PROFILE
// global for testing how many comparisons are performed for each sorting
// algorithm
...
struct Range {
  ...
  std::size_t length() const { return std::distance(start, end); }
};
...
// comparisons in situations where have some idea as to how many unique values
// there are and where the next value might be
...
RandomAccessIterator FindFirstBackward(RandomAccessIterator first,
  ...
  RandomAccessIterator index;
  for (index = last - skip; index > first && !compare(*(index - 1), value);
       index -= skip) {
    if (index < first + skip) {
...
RandomAccessIterator FindLastBackward(RandomAccessIterator first,
  ...
  RandomAccessIterator index;
  for (index = last - skip; index > first && compare(value, *(index - 1));
       index -= skip) {
    if (index < first + skip) {
...
void MergeInternal(RandomAccessIterator first1,
  ...
  // that's already in that spot when this algorithm is finished, 'buffer' will
  // contain its original contents, but in a different order
...
class Iterator {
 ...
 public:
  ...
  std::size_t length() const { return decimal_step; }
};

#if DYNAMIC_CACHE
// use a class so the memory for the cache is freed when the object goes out of
// scope, regardless of whether exceptions were thrown (only needed in the C++
// version)
template <typename T>
class Cache {
 public:
  ...
  Cache(std::size_t size) {
    ...
    // sqrt((size + 1)/2) + 1 – this will be the size of the A blocks at the
    // largest level of merges, so a buffer of this size would allow it to skip
    // using internal or in-place merges for anything
    cache_size = std::sqrt(cache_size) + 1;
    cache = new (std::nothrow) T[cache_size];
    if (cache)
      return;

    // 512 – chosen from careful testing as a good balance between fixed-size
...
void Sort(RandomAccessIterator first, RandomAccessIterator last) {
  Sort(first, last, [](const auto& a, const auto& b) -> bool { return a < b; });
}
...
void Sort(RandomAccessIterator first,
          RandomAccessIterator last,
          Comparison compare) {
  // map first and last to a C-style array, so we don't have to change the rest
  // of the code (bit of a nasty hack, but it's good enough for now...)
...
// use a small cache to speed up some of the operations
  ...
  // fit into it), and making it too large also ruins the point (so much for
  // "low memory"!) removing the cache entirely still gives 75% of the
  // performance of a standard merge
  ...
  while (true) {
    ...
    // specifically for merging with the cache (we use < rather than <= since
    // the block size might be one more than iterator.length())
    if (iterator.length() < cache_size) {
      // if four subarrays fit into the cache, it's faster to merge both pairs
      // of subarrays into the cache, then merge the two merged subarrays from
      // the cache back into the original array
      if ((iterator.length() + 1) * 4 <= cache_size &&
          iterator.length() * 4 <= size) {
        iterator.begin();
        while (!iterator.finished()) {
          ...
          if (compare(*(B1.end - 1), *A1.start)) {
            // the two ranges are in reverse order, so copy them in reverse
            // order into the cache
            std::copy(A1.start, A1.end, cache + B1.length());
            std::copy(B1.start, B1.end, cache);
          } else if (compare(*B1.start, *(A1.end - 1))) {
            // these two ranges weren't already in order, so merge them into the
            // cache
          ...
          if (compare(*(B2.end - 1), *A2.start)) {
            ...
            // these two ranges weren't already in order, so merge them into the
            // cache
            std::merge(A2.start, A2.end, B2.start, B2.end, cache + A1.length(),
                       compare);
          } else {
          ...
          if (compare(*(B3.end - 1), *A3.start)) {
            ...
            // these two ranges weren't already in order, so merge them back
            // into the array
        ...
        // we merged two levels at the same time, so we're done with this level
        // already (iterator.nextLevel() is called again at the bottom of this
        // outer merge loop)
        iterator.nextLevel();
        ...
        while (!iterator.finished()) {
          ...
          if (compare(*(B.end - 1), *A.start)) {
            // the two ranges are in reverse order, so a simple rotation should
            // fix it
      ...
      //     1a. adjust block_size and buffer_size if we couldn't find enough
      //     unique values
      // 2. loop over the A and B subarrays within this level of the merge sort
      //     3. break A and B into blocks of size 'block_size'
      //     4. "tag" each of the A blocks with values from the first internal
      //     buffer
      //     5. roll the A blocks through the B blocks and drop/rotate them
      //     where they belong
      //     6. merge each A block with any B values that follow, using the
      //     cache or the second internal buffer
      ...
      // as an optimization, we really only need to pull out the internal
      // buffers once for each level of merges after that we can reuse the same
      // buffers over and over, then redistribute it when we're finished with
      // this level
      ...
      // let's try finding both buffers at the same time from a single A or B
      // subarray
      std::size_t find = buffer_size + buffer_size;
      bool find_separately = false;

      if (block_size <= cache_size) {
        // if every A block fits into the cache then we won't need the second
        // internal buffer, so we really only need to find 'buffer_size' unique
        // values
        find = buffer_size;
      } else if (find > iterator.length()) {
        // we can't fit both buffers into the same A or B subarray, so find two
        // buffers separately
        find = buffer_size;
        find_separately = true;
      }

      // we need to find either a single contiguous space containing 2√A unique
      // values (which will be split up into two buffers of size √A each), or we
      // need to find one buffer of < 2√A unique values, and a second buffer of
      // √A unique values, OR if we couldn't find that many unique values, we
      // need the largest possible buffer we can get

      // in the case where it couldn't find a single buffer of at least √A
      // unique values, all of the Merge steps must be replaced by a different
      // merge algorithm (MergeInPlace)
...
#define PULL(_to)                                                       \
  ...
  pull[pull_index].to = _to
        ...
        // check A for the number of unique values we need to fill an internal
        // buffer these values will be pulled out to the start of A
        for (last = A.start, count = 1; count < find; last = index, ++count) {
          index =
              FindLastForward(last + 1, A.end, *last, compare, find - count);
          if (index == A.end)
            break;
          assert(index < A.end);
        }
        ...
        if (count >= buffer_size) {
          // keep track of the range within the array where we'll need to "pull
          // out" these values to create the internal buffer
          PULL(A.start);
          pull_index = 1;

          if (count == buffer_size + buffer_size) {
            // we were able to find a single contiguous section containing 2√A
            // unique values, so this section can be used to contain both of the
            // internal buffers we'll need
            buffer1 =
                Range<RandomAccessIterator>(A.start, A.start + buffer_size);
            buffer2 = Range<RandomAccessIterator>(A.start + buffer_size,
                                                  A.start + count);
            break;
          } else if (find == buffer_size + buffer_size) {
            // we found a buffer that contains at least √A unique values, but
            // did not contain the full 2√A unique values, so we still need to
            // find a second separate buffer of at least √A unique values
            buffer1 = Range<RandomAccessIterator>(A.start, A.start + count);
            find = buffer_size;
          } else if (block_size <= cache_size) {
            // we found the first and only internal buffer that we need, so
            // we're done!
            buffer1 = Range<RandomAccessIterator>(A.start, A.start + count);
            break;
            ...
            // we found a second buffer in an 'A' subarray containing √A unique
        ...
        // check B for the number of unique values we need to fill an internal
        // buffer these values will be pulled out to the end of B
        for (last = B.end - 1, count = 1; count < find;
             last = index - 1, ++count) {
          index =
              FindFirstBackward(B.start, last, *last, compare, find - count);
          if (index == B.start)
            break;
        ...
        if (count >= buffer_size) {
          ...
          if (count == buffer_size + buffer_size) {
            ...
            // internal buffers we'll need
            buffer1 =
                Range<RandomAccessIterator>(B.end - count, B.end - buffer_size);
            ...
            // buffer2 will be pulled out from a 'B' subarray, so if the first
            // buffer was pulled out from the corresponding 'A' subarray, we
            // need to adjust the end point for that A subarray so it knows to
            // stop redistributing its values before reaching buffer2
            if (pull[0].range.start == A.start) {
              pull[0].range.end -= pull[1].count;
            }

            // we found a second buffer in a 'B' subarray containing √A unique
            // values, so we're done!
...
#undef PULL
      ...
      for (pull_index = 0; pull_index < 2; ++pull_index) {
        ...
        if (pull[pull_index].to < pull[pull_index].from) {
          // we're pulling the values out to the left, which means the start of
          // an A subarray
          index = pull[pull_index].from;
          for (count = 1; count < length; ++count) {
            index = FindFirstBackward(pull[pull_index].to,
                                      pull[pull_index].from - (count - 1),
                                      *(index - 1), compare, length - count);
            Range<RandomAccessIterator> range(index + 1,
                                              pull[pull_index].from + 1);
            std::rotate(range.start, range.end - count, range.end);
            pull[pull_index].from = index + count;
          }
        } else if (pull[pull_index].to > pull[pull_index].from) {
          // we're pulling values out to the right, which means the end of a B
          // subarray
          index = pull[pull_index].from + 1;
          for (count = 1; count < length; ++count) {
            index = FindLastForward(index, pull[pull_index].to, *index, compare,
                                    length - count);
      ...
      // adjust block_size and buffer_size based on the values we were able to
      // pull out
      buffer_size = buffer1.length();
      block_size = iterator.length() / buffer_size + 1;

      // the first buffer NEEDS to be large enough to tag each of the evenly
      // sized A blocks, so this was originally here to test the math for
      // adjusting block_size above
      ...
      while (!iterator.finished()) {
        ...
        // remove any parts of A or B that are being used by the internal
        // buffers
        ...
        if (start == pull[0].range.start) {
          if (pull[0].from > pull[0].to) {
            ...
            // if the internal buffer takes up the entire A or B subarray, then
            // there's nothing to merge this only happens for very small
            // subarrays, like √4 = 2, 2 * (2 internal buffers) = 4, which also
            // only happens when cache_size is small or 0 since it'd otherwise
            // use MergeExternal
            if (A.length() == 0)
              continue;
        ...
        if (compare(*(B.end - 1), *A.start)) {
          ...
          Range<RandomAccessIterator> firstA(
              A.start, A.start + blockA.length() % block_size);
          ...
          for (RandomAccessIterator indexA = buffer1.start, index = firstA.end;
               index < blockA.end; ++indexA, index += block_size) {
            std::iter_swap(indexA, index);
          }

          // start rolling the A blocks through the B blocks!
          // when we leave an A block behind we'll need to merge the previous A
          // block with any B blocks that follow it, so track that information
          // as well
          Range<RandomAccessIterator> lastA(firstA);
          Range<RandomAccessIterator> lastB(first, first);
          Range<RandomAccessIterator> blockB(
              B.start, B.start + std::min(block_size, B.length()));
          blockA.start += firstA.length();
          RandomAccessIterator indexA = buffer1.start;

          // if the first unevenly sized A block fits into the cache, copy it
          // there for when we go to Merge it otherwise, if the second buffer is
          // available, block swap the contents into that
          ...
          if (blockA.length() > 0) {
            while (true) {
              // if there's a previous B block and the first value of the
              // minimum A block is <= the last value of the previous B block,
              // then drop that minimum A block behind. or if there are no B
              // blocks left then keep dropping the remaining A blocks.
              if ((lastB.length() > 0 && !compare(*(lastB.end - 1), *indexA)) ||
                  blockB.length() == 0) {
                // figure out where to split the previous B block, and rotate it
                // at the split
                ...
                // swap the minimum A block to the beginning of the rolling A
                // blocks
                RandomAccessIterator minA = blockA.start;
                for (RandomAccessIterator findA = minA + block_size;
                     findA < blockA.end; findA += block_size) {
                ...
                // follow it if lastA fits into the external cache we'll use
                // that (with MergeExternal), or if the second internal buffer
                // exists we'll use that (with MergeInternal), or failing that
                // we'll use a strictly in-place merge algorithm (MergeInPlace)
                if (lastA.length() <= cache_size) {
                  MergeExternal(lastA.start, lastA.end, lastA.end, B_split,
                                cache, compare);
                } else if (buffer2.length() > 0) {
                  MergeInternal(lastA.start, lastA.end, lastA.end, B_split,
                                buffer2.start, compare);
                } else {
                  MergeInPlace(lastA.start, lastA.end, lastA.end, B_split,
                               compare);
                }
                ...
                if (buffer2.length() > 0 || block_size <= cache_size) {
                  ...
                  if (block_size <= cache_size) {
                    ...
                    std::swap_ranges(blockA.start, blockA.start + block_size,
                                     buffer2.start);
                  }
                  ...
                  // memcopied it either way we don't need to retain the order
                  // of those items, so instead of rotating we can just block
                  // swap B to where it belongs
                  std::swap_ranges(B_split, B_split + B_remaining,
                                   blockA.start + block_size - B_remaining);
                } else {
                  // we are unable to use the 'buffer2' trick to speed up the
                  // rotation operation since buffer2 doesn't exist, so perform
                  // a normal rotation
                ...
                lastB = Range<RandomAccessIterator>(lastA.end,
                                                    lastA.end + B_remaining);

                // if there are no more A blocks remaining, this step is
                // finished!
                ...
                // roll the leftmost A block to the end by swapping it with the
                // next B block
                std::swap_ranges(blockA.start, blockA.start + block_size,
                                 blockB.start);
                lastB = Range<RandomAccessIterator>(blockA.start,
                                                    blockA.start + block_size);
          ...
          if (lastA.length() <= cache_size) {
            MergeExternal(lastA.start, lastA.end, lastA.end, B.end, cache,
                          compare);
          } else if (buffer2.length() > 0) {
      ...
      // insertion sort the second buffer, then redistribute the buffers back
      // into the array using the opposite process used for creating the buffer

      // while an unstable sort like std::sort could be applied here, in
      // benchmarks it was consistently slightly slower than a simple insertion
      // sort, even for tens of millions of items. this may be because insertion
      // sort is quite fast when the data is already somewhat sorted, like it is
      // here
      ...
      for (pull_index = 0; pull_index < 2; ++pull_index) {
        ...
        if (pull[pull_index].from > pull[pull_index].to) {
          // the values were pulled out to the left, so redistribute them back
          // to the right
          ...
          // the values were pulled out to the right, so redistribute them back
          // to the left
          Range<RandomAccessIterator> buffer(
