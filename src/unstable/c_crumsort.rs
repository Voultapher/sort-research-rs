...
extern "C" {
    fn crumsort_unstable_i32(data: *mut i32, len: usize);
    #[must_use]
    fn crumsort_unstable_i32_by(
        data: *mut i32,
        len: usize,
    ...
    fn crumsort_unstable_u64(data: *mut u64, len: usize);
    #[must_use]
    fn crumsort_unstable_u64_by(
        data: *mut u64,
        len: usize,
...
impl CrumSort for i32 {
    fn sort(data: &mut [i32]) {
        unsafe {
            crumsort_unstable_i32(data.as_mut_ptr(), data.len());
        }
    }

    fn sort_by<F: FnMut(&i32, &i32) -> Ordering>(data: &mut [i32], compare: F) {
        make_cpp_sort_by!(crumsort_unstable_i32_by, data, compare, i32);
    }
}

impl CrumSort for u64 {
    fn sort(data: &mut [u64]) {
        unsafe {
            crumsort_unstable_u64(data.as_mut_ptr(), data.len());
        }
    }

    fn sort_by<F: FnMut(&u64, &u64) -> Ordering>(data: &mut [u64], compare: F) {
        make_cpp_sort_by!(crumsort_unstable_u64_by, data, compare, u64);
    }
}
